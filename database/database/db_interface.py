...
# Ensure Base is consistent across your models.
# This should ideally be defined once in a central models file or engine file.
Base = declarative_base()

class DBInterface:
    _engine = None  # Class attribute to hold the async engine instance
    AsyncSessionLocal = None # Class attribute to hold the async session factory
    ...
    def initialize_engine_and_session(cls, database_url: str):
        ...
        """
        if not database_url:
            raise ValueError("DATABASE_URL must be provided for engine initialization.")
    ...
    def __init__(self, model: Type[Base]):
        ...
        # No more engine/sessionmaker creation here!
    ...
    async def create(self, data: Dict[str, Any]) -> Base:
        ...
        async with self.AsyncSessionLocal() as session:
            async with session.begin(): # Use begin() for transactional safety
                item = self.model(**data)
                session.add(item)
            await session.refresh(item) # Refresh to get any database-generated values (e.g., IDs)
            return item

    async def read_by_name(self, name: str) -> Optional[Base]:
        """
        Reads a record by its 'player_name' (assuming this is a unique identifier).
        This is typically used for the Player model.
        """
        async with self.AsyncSessionLocal() as session:
    ...
    async def read_by_link(self, link_id: int) -> Optional[Base]:
        """
        Reads a record by its 'link' (assuming this is a unique identifier).
        This is typically used for the Game and Move models.
        """
        async with self.AsyncSessionLocal() as session:
            result = await session.execute(
    ...
    async def update_by_name(self, name: str, new_data: Dict[str, Any]) -> Optional[Base]:
        """
        Updates a record identified by 'player_name' with new data.
        This is typically used for the Player model.
        """
        async with self.AsyncSessionLocal() as session:
    ...
    def to_dict(self, obj: Base) -> Dict[str, Any]:
        """
        Converts a SQLAlchemy ORM object to a dictionary, handling relationships if needed.
...
# # Assuming DATABASE_URL is set in your environment variables
# DATABASE_URL = CONN_STRING

# # Make sure your Base is the same one used for defining your models
# Base = declarative_base()
...
#         # Initialize engine only once per process, or manage it globally
#         # This assumes you have a way to ensure the engine is truly global
#         # For a simple script, initializing here is fine, but for a web app,
#         # you might manage this with FastAPI's lifespan events or similar.
#         if not hasattr(self.__class__, '_engine') or self.__class__._engine is None:
#             self.__class__._engine = create_async_engine(DATABASE_URL, echo=False)
#             self.__class__.AsyncSessionLocal = sessionmaker(
#                 self.__class__._engine, expire_on_commit=False, class_=AsyncSession
#             )
